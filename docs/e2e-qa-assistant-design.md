...
Harness 会话的“端到端 QA”页签提供四个视图：

- 总览：项目数量、配置状态、校验结果、缺失环境变量。
- 环境向导：选择自动识别的项目，配置页面/API、登录、MySQL 和日志连接引用。
- 测试用例：展示结构化 suite/case、风险等级和步骤数量，选择后执行。
- 运行报告：展示通过率、用例覆盖率、失败问题和报告路径。

工作台不会创建第二份配置。向导生成或修改的仍是工作区 `.dsh/qa.e2e.json`；Agent 工具、任务看板和页面直接运行共享同一事实源。工作台 API 的工作区由 Host 根据当前 live session 解析，固定写入该工作区，拒绝非 loopback 请求和并发运行。
